...
int main(int argc, char * argv[]){
    ...
    float right_rotations = (float)(drivetrain.right_motor->encoder->ticks/(44.0*21.3));
    float distance = 2.0;
    while(!moveComplete && right_rotations <= distance*2){ 
        uint32_t current_time = gpioTick();
        right_rotations = (float)(drivetrain.right_motor->encoder->ticks/(44.0*21.3));
        printf("(%f | %f)\n", 0.0, right_rotations);
        drivetrain_spin(&drivetrain, 0, (int)pid_power(&pid_right, distance, right_rotations, current_time));
        if(fabs(pid_right.error)<=pid_right.error_tolerance){ timesGood++; }
